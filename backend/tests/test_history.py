...
class TestHistoryModule:
    ...
    def _with_temp_path(self, func):
        """在临时路径下执行函数"""
        with patch('backend.engine.cp_engine.history.HISTORY_DIR', self.temp_dir), \
             patch('backend.engine.cp_engine.history.HISTORY_FILE', self.temp_file), \
             patch('backend.engine.cp_engine.history._get_cp_store', return_value=None):
            return func()
