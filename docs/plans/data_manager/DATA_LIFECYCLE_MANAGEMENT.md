# TradeSnake 数据生命周期管理方案 v1.7

> **v1.7更新**：补充预测引擎数据存储（gain_predictions, probability_predictions，90天保留）；修正分钟K线保留期为14天
> **v1.6更新**：修正 price_history 表用途说明，明确为回测器使用的历史遗留表
> **v1.5更新**：新增历史数据填充功能说明（十五章）
> **v1.4更新**：新增回测报告存档规范（backtest_reports）
> **v1.3更新**：根据股票池规划（核心300/活跃500/观察1000/全市场5000）调整保留期限：
> - 战力历史：永久 → **2年**（约44MB）
> - 日K线：5年 → **2年**（全市场5000只，约200MB）
...
│  │ price_history│ 价格历史（回测器使用，**历史遗留表**）          │  │
│  │ alert_config│ 告警配置                                          │  │
│  │ alerts      │ 告警记录                                          │  │
│  │ user_profile│ 用户配置                                          │  │
│  │ config      │ 系统配置                                          │  │
...
│  │   表名              │  说明                                     │ │
│  ├─────────────────────┼─────────────────────────────────────────┤ │
│  │ gain_predictions    │ 涨幅预测结果（P2，90天保留）             │ │
│  │ probability_predictions │ 上涨概率预测（P2，90天保留）          │ │
│  └─────────────────────┴─────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────┘
...
| **P2 重要参考** | 日K线历史、预测结果 | DuckDB/SQLite | **2年**(K线)/**90天**(预测) | 约200MB+历史K线 |
...
# 配置项合法值约束
CONFIG_CONSTRAINTS = {
    'daily_kline_years': {'min': 1, 'max': 5, 'default': 2},
    'minute_kline_days': {'min': 7, 'max': 14, 'default': 14},
    'prediction_retention_days': {'min': 30, 'max': 180, 'default': 90},
...
| 配置项 | 默认值 | 可选范围 | 说明 |
|-------|-------|---------|------|
| 日K保留年数 | 2年 | 1/2/3/5年 | DuckDB |
| 分钟K保留天数 | 14天 | 7/14天 | DuckDB（上限14天） |
| 预测结果保留 | 90天 | 30/60/90/180天 | SQLite |
...
| 评审4 | price_history冗余问题 | ⚠️ 保留（回测器使用，**计划迁移到DuckDB**） |
...
| 涨幅预测 | SQLite | 90天 | 180天 | ✅ | 每日02:00 |
| 上涨概率预测 | SQLite | 90天 | 180天 | ✅ | 每日02:00 |
