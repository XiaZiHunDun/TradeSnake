...
> - 依据：414天 Alpha 分析，growth IC=+0.0104(t=2.46) 唯一显著正因子
> - v19.8 旧权重（growth=30%, value=25%, quality=20%, momentum=8%）已废弃
> - 实际权重配置见 `backend/engine/cp_engine/constants.py` 的 `WEIGHTS` 字典

## 概述

分析引擎模块是 TradeSnake 系统的核心计算模块，负责股票战力计算、风险评估、历史追踪，以及新增的预测分析。

**产品范围**：与全项目一致，战力主算与 `stock_selector` 协同的分析集合以 **沪深主板** 为边界；详见 `../PROJECT_OVERVIEW.md`「产品范围」。
...
```
总战力 = (成长分×50% + 动量分×28% + 质量分×5%) × 风险惩罚调整 + 实时分×2%
```
> ⚠️ 注意：v19.8 旧权重（成长30%+价值25%+质量20%+动量8%）已废弃，以 v21 权重为准。

**各因子说明**：

| 因子 | 权重 | 数据来源 | 计算方式 |
|------|------|----------|----------|
| 成长分 | 50% | 财务数据 | 净利润增速(60%) + 营收增速(40%) |
| 价值分 | 0% | 行情+财务 | IC=-0.009反转，v21已排除 |
| 质量分 | 5% | 财务数据 | 现金流 + 毛利率 + 资产负债率 |
| 动量分 | 28% | 行情数据 | 多日动量(60%) + 当日涨跌幅(40%) |
| 实时分 | 2% | 分钟级数据 | MA5变化(50%) + MA15变化(30%) + 成交量(20%) |
| 风险惩罚 | 10% | 综合评估 | 根据PE、ROE、波动率等调整（乘数） |
