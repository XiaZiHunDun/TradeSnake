...
| 板块 | 代码前缀 | 实际限制 | 判断阈值 |
|------|----------|----------|----------|
| 主板 | 000/600/601等 | 10% | ≥9.9% / ≤-9.9% |
| 创业板 | 300 | 20% | ≥19.9% / ≤-19.9% |
| 科创板 | 688 | 20% | ≥19.9% / ≤-19.9% |
| 北交所 | 8开头 | 30% | ≥29.9% / ≤-29.9% |

> 注：判断阈值 = 实际限制 - 0.1%，用于识别是否接近涨跌停。产品范围仅沪深主板。
...
| 状态 | 判断条件 | 处理逻辑 |
|------|----------|----------|
| 涨停 | change_pct ≥ 板块阈值-0.1% | 预测涨幅限制max(predicted, 5%)，避免过度乐观 |
| 跌停 | change_pct ≤ -(板块阈值-0.1%) | 预测涨幅限制min(predicted, -3%)，避免过度悲观 |
| 正常 | 其他 | 截断到板块涨跌幅限制范围内 |

> 注：板块阈值见5.2节（主板10%、创业板/科创板20%、北交所30%）。涨跌停判断使用阈值-0.1%，与实际涨跌停价格（±9.9%等）保持一致。
