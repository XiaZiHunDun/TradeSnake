...
| 文件 | 内容 | 行数 |
|------|------|------|
| [RECOMMENDER_DETAIL.md](./RECOMMENDER_DETAIL.md) | 详细设计（数据模型、接口契约、版本历史、相关文档） | ~320 |
| **本文档** | 概述、输入输出、三大场景、模块结构、核心组件、风控清单 | ~398 |

## 内容速览

### RECOMMENDER_OVERVIEW.md（本文档）
...
- **预测融合**（v19.8）：归一化公式(cp/100, gain/50)、融合权重(保守0.5/0.3/0.2, 平衡0.4/0.35/0.25, 激进0.3/0.4/0.3)
- **模块结构**：recommend_engine/swap_calculator/buy_analyzer/sell_analyzer/filters/fusion/prompts
- **核心组件**：RecommendEngine（三大建议入口）、SwapCalculator（五档决策矩阵）、BuyAnalyzer（Kelly仓位/止损止盈）、SellAnalyzer（盈亏状态/紧急程度）
...
class BuySignal:
    ...
    stop_loss: float           # 固定止损价（-7%，对应 RiskManager 固定止损；尾随止损-8%由 RiskManager 自动执行）
...
def analyze_buy_opportunity(
    ...
    # 3. 止损止盈
    stop_loss = stock.price * 0.93        # 固定止损-7%（尾随止损-8%由 RiskManager 自动执行）
